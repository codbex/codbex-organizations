angular.module('page', ['blimpKit', 'platformView', 'EntityService'])
	.config(['EntityServiceProvider', (EntityServiceProvider) => {
		EntityServiceProvider.baseUrl = '/services/ts/codbex-organizations/gen/codbex-organizations/api/organizations/DepartmentService.ts';
	}])
	.controller('PageController', ($scope, $http, EntityService, Extensions, ButtonStates) => {
		...
		Extensions.getWindows(['codbex-organizations-custom-action']).then((response) => {
			$scope.pageActions = response.data.filter(e => e.perspective === 'organizations' && e.view === 'Department' && (e.type === 'page' || e.type === undefined));
			$scope.entityActions = response.data.filter(e => e.perspective === 'organizations' && e.view === 'Department' && e.type === 'entity');
		});
		...
		Dialogs.addMessageListener({ topic: 'codbex-organizations.organizations.Department.clearDetails', handler: () => {
			$scope.$evalAsync(() => {
				$scope.entity = {};
				$scope.action = 'select';
			});
		}});
		Dialogs.addMessageListener({ topic: 'codbex-organizations.organizations.Department.entityCreated', handler: () => {
			$scope.loadPage($scope.dataPage, $scope.filter);
		}});
		Dialogs.addMessageListener({ topic: 'codbex-organizations.organizations.Department.entityUpdated', handler: () => {
			$scope.loadPage($scope.dataPage, $scope.filter);
		}});
		Dialogs.addMessageListener({ topic: 'codbex-organizations.organizations.Department.entitySearch', handler: (data) => {
			resetPagination();
		...
		$scope.loadPage = (pageNumber, filter) => {
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						title: 'Department',
						message: `Unable to list/filter Department: '${message}'`,
				...
				Dialogs.showAlert({
					title: 'Department',
					message: `Unable to count Department: '${message}'`,
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						$scope.loadPage($scope.dataPage, $scope.filter);
						Dialogs.triggerEvent('codbex-organizations.organizations.Department.clearDetails');
					}, (error) => {
						const message = error.data ? error.data.message : '';
						Dialogs.showAlert({
							title: 'Department',
